...
export const createCourt = handleAsync(async (req, res, next) => {
  ...
  return res
    .status(201)
    .json(createResponse(true, 201, "Tạo sân thành công!", court));
});
...
export const getDetailCourt = handleAsync(async (req, res, next) => {
  ...
  if (!court)
    return res.json(
      createResponse(true, 200, "Lấy chi tiết sân thành công !", court, null)
    );
...
export const updateCourt = handleAsync(async (req, res, next) => {
  ...
  if (updateData.code) {
    ...
    if (existing)
      return next(createError(false, 400, " Mã sân không tồn tại !"));
  }
  ...
  //lấy lại amenities
  const updateAmentity = await CourtAmenity.find({ courtId: court._id })
    .select("name")
    .lean();
  return res.json(
