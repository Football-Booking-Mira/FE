...
export default function CourtForm({ open, onClose, court, onSuccess }: CourtFormProps) {
    ...
    const handleSubmit = async () => {
        try {
            ...
            toast.success(court ? 'Cập nhật sân thành công!' : 'Thêm sân mới thành công!');
            onSuccess();
            onClose();
        } catch (err: any) {
    ...
    return (
        <Dialog open={open} onOpenChange={(val) => !val && onClose()}>
            <DialogContent className='max-w-2xl'>
                <DialogHeader>
                    <DialogTitle>{court ? 'Sửa thông tin sân' : 'Thêm sân mới'}</DialogTitle>
                </DialogHeader>
