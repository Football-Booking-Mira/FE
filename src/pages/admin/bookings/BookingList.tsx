...
const getRefundActionOptions = (
  current: Booking["refundStatus"] | undefined
) => {
...
export default function BookingList() {
  ...
  const [refundModalMode, setRefundModalMode] = useState<"account" | "admin">(
    "account"
  );
  ...
  const [checkinMode, setCheckinMode] = useState<"checkin" | "add_equipment">(
    "checkin"
  );
  const [equipmentList, setEquipmentList] = useState<EquipmentItem[]>([]);
  const [equipmentQty, setEquipmentQty] = useState<Record<string, number>>({});
  const [equipmentBaseQty, setEquipmentBaseQty] = useState<
  ...
  const loadCheckinEquipments = async (bookingId?: string) => {
    try {
      ...
      if (bookingId) {
        try {
          const itemsRes = await api.get(
            `/bookings/${bookingId}/equipments-detail`
          );
      ...
      rawEquipments.forEach((e: any) => {
        ...
        const stock =
          e.availableQuantity ?? e.stockLeft ?? e.stock ?? e.totalQuantity ?? 0;
  ...
  const openCheckinModal = async (
    b: Booking,
    mode: "checkin" | "add_equipment" = "checkin"
  ) => {
    setCheckinBooking(b);
    setCheckinMode(mode);
  ...
  const handleConfirmCheckin = async () => {
    ...
    try {
      ...
      const msg =
        err?.response?.data?.message || "Lỗi check-in / cập nhật thiết bị!";
      toast.error(msg);
    }
  };
    ...
    try {
      ...
      const list = rawList.map((b) => {
        const hasDepositPaid =
          (b.depositAmount || 0) > 0 && b.depositStatus === "paid";
        const deposit = Number(b.depositAmount || 0);
        const total = Number(b.total || 0);

        let paymentStatus: Booking["paymentStatus"] =
          (b.paymentStatus as any) || "unpaid";
    ...
    Modal.info({
      ...
      content: (
        <div>
          <p>
            <b>Khách hàng:</b>{" "}
            {b.customerInfo?.name || b.customerId?.name || "Ẩn danh"}
          </p>
          {b.cancelReason ? (
  ...
  const openBookingDetailModal = async (bookingId: string) => {
    ...
    try {
      ...
      const msg =
        err?.response?.data?.message || "Không thể tải chi tiết đơn đặt sân!";
      toast.error(msg);
    ...
    Modal.confirm({
      ...
      onOk: async () => {
        try {
          ...
          const msg =
            err?.response?.data?.message || "Lỗi khi hủy đơn tiền mặt!";
          toast.error(msg);
    ...
    try {
      ...
      try {
        ...
        const msg =
          ...
          "Lỗi tạo hóa đơn / cập nhật thanh toán!";
        toast.error(msg);
      } finally {
  ...
  const handleRefundStatusChange = async (
    ...
    try {
      ...
      const msg =
        err?.response?.data?.message || "Lỗi cập nhật trạng thái hoàn tiền!";
      toast.error(msg);
    }
  };
  ...
  const filteredBookings = bookings.filter((b) => {
    ...
    const matchesDate = filters.date
      ? dayjs(b.date).isSame(filters.date, "day")
      : true;
    return matchesSearch && matchesStatus && matchesDate;
  });

  const refundBase = bookings.filter(
    (b) => b.refundStatus && b.refundStatus !== "none"
  );
  ...
  const filteredRefundBookings = refundBase.filter((b) => {
    ...
    const matchesRefundFilter =
      refundFilter === "all" ? true : rs === refundFilter;
  ...
  const bookingColumns = [
    ...
    },
    {
      ...
      key: "time",
      render: (
        b: Booking & { slots?: { startTime: string; endTime: string }[] }
      ) => {
  ...
  const paymentFinalTotal = Math.max(
    0,
    paymentBaseTotal - (Number(paymentDiscount || 0) || 0)
  );
  ...
  const billImage =
    selectedRefundBooking?.refund?.billImage ||
    selectedRefundBooking?.refundBillImage ||
    "";
    ...
    <div className="space-y-6">
                ...
                <>
                  <div className="flex flex-wrap gap-4 mb-4">
                    ...
                    <Select
                      ...
                      allowClear
                      value={refundFilter === "all" ? undefined : refundFilter}
                      onChange={(v) =>
                      ...
                      <div className="flex justify-between items-center">
                        <div>
                          <div className="font-medium text-sm">
                            {item.name}{" "}
                            <Tag
                              color={item.mode === "rent" ? "blue" : "green"}
                              style={{ marginLeft: 4 }}
                            >
                          ...
                          <div className="text-xs text-gray-500">
                            Giá: {formatVND(item.price)} / {item.unit || "cái"}
                          </div>
                          <div className="text-xs text-gray-500">
                          ...
                          {qty > 0 && (
                            <div className="text-xs text-gray-600 mt-1">
                              Thành tiền: <b>{formatVND(lineTotal)}</b>
                            </div>
                          )}
                        </div>
                        ...
                        <div className="flex items-center gap-2">
                          <Button
                            size="small"
                            onClick={() =>
                              changeEquipmentQty(item.key, -1, item.stock)
                            }
                            disabled={qty <= base}
                          >
                            -
        ...
        {invoiceLoading ? (
          ...
          (() => {
            ...
            const customer =
              ...
              {};
            const methodLabel =
              PAYMENT_METHOD_TEXT[inv.method] || inv.method || "—";
                  ...
                  >
                    ...
                    <div className="text-xs space-y-1">
                      <div className="flex justify-between">
                        ...
                        <span className="font-medium">
                          {customer.name || customer.username || "Khách lẻ"}
                        </span>
                      </div>
                      {customer.phone && (
                        <div className="flex justify-between">
                  ...
                  <div className="space-y-1 text-sm">
                    ...
                    {voucherDiscount > 0 && booking.voucherCode && (
                      <div className="flex justify-between text-green-600">
                        <span>Giảm giá voucher ({booking.voucherCode})</span>
                        <span className="font-medium">
                          - {formatVND(voucherDiscount)}
                        </span>
                      </div>
      ...
      >
        <p className="mb-2 text-sm text-gray-600">
          Lý do này sẽ được hiển thị cho khách hàng ở trang &quot;Đơn của
          tôi&quot;.
        </p>
        <TextArea
          rows={4}
      ...
      >
        <p className="mb-2 text-sm text-gray-600">
          Chọn ảnh hoá đơn / bill hoàn tiền (sau khi chuyển khoản cho khách).
          Ảnh này sẽ hiển thị cho khách hàng khi xem chi tiết đơn hoàn tiền.
        </p>
        <Upload
          listType="picture"
          maxCount={1}
